...
class Match:
    ...
    def tournament(self, input_tournament):
        if self.tournament is not None:
            raise MatchError(Match.THE_TOURNAMENT_OF_A_MATCH_CANNOT_BE_MODIFIED)
        if not class_id_strings.check_class_id(input_tournament, class_id_strings.TOURNAMENT_ID):
            raise MatchError(Match.INVALID_TOURNAMENT_FOR_A_MATCH)
    ...
    def score(self, input_score):
        int_input_score = self._check_valid(input_score)
    ...
    def _check_valid(self, score):
        ...
        if len(score) > n_sets:
            raise MatchError(Match.INVALID_NUMBER_OF_SETS_FOR_A_MATCH)
        set_player_1 = set_player_2 = 0
        won = False
        int_score = []
        for n_set, set_score in enumerate(score):
            if won:
                ...
                raise MatchError(Match.INVALID_NUMBER_OF_SETS_FOR_A_MATCH)
            if len(set_score) != 2:
                raise MatchError(Match.INVALID_SCORE_FOR_A_SET)
            try:
    ...
    @staticmethod
    def _is_valid_set_at_6(set_score):
    ...
    @staticmethod
    def _is_valid_set_at_tiebreaker(set_score, tie_breaker_score):
