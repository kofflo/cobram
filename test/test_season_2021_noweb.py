...
def test_season_2021():
    ...
    for tournament_data_index, tournament in enumerate(tournaments):
        ...
        print(list((environment.get_gambler_info(index)['nickname'], score) for index, score in tournament_ranking['tournament_scores'].items()))
        print(list((environment.get_gambler_info(index)['nickname'], score) for index, score in tournament_ranking['tournament_ranking_scores'].items()))
        league_ranking = environment.get_league_ranking(**{'league_index': coppa_cobram_index})
        print(list((environment.get_gambler_info(index)['nickname'], score) for index, score in league_ranking['ranking_scores'].items()))
        print(league_ranking['yearly_scores'])
        print(league_ranking['winners'])
        print(league_ranking['last_tournament'])
    ...
    environment.deactivate_gambler_from_league(coppa_cobram_index, macchia_index)
    ...
    tournament_ranking = environment.get_tournament_ranking(**{'league_index': coppa_cobram_index, 'tournament_id': ("Australian Open", 2022)})
    ...
    tm = environment.get_tournament_matches(league_index=0, tournament_id=("Australian Open", 2021), gambler_index=None)
    for ttm in tm:
        print(ttm)
        print(tm[ttm])
    tm = environment.get_tournament_matches(league_index=0, tournament_id=("Australian Open", 2021), gambler_index=1)
    for ttm in tm:
        print(ttm)
        print(tm[ttm])

    print(environment.get_tournament_ranking(league_index=coppa_cobram_index, tournament_id=('Wimbledon Championships', 2021)))
