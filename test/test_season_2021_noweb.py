...
def tournament_id_to_index(league_index, tournament_id):
    return environment._get_league(league_index).get_tournament_index(tournament_id=tournament_id)
...
def add_gamblers_to_league(league_index, *gamblers):
    for ((nickname, _), initial_score, *_) in gamblers:
        ...
        environment.add_gambler_to_league(**{'league_index': league_index, 'gambler_index': gambler_index,
                                             'initial_score': initial_score, 'initial_credit': 0})
...
def set_round_bets(league_index, tournament, gambler_index, round_index, scores, joker):
    ...
    tournament_index = tournament_id_to_index(league_index, (name, year))
    info = environment.get_tournament_info(**{'league_index': league_index, 'tournament_index': tournament_index})
...
def test_season_2021():
    ...
    coppa_cobram_index = list(environment.create_league(name="Coppa Cobram").keys())[0]
    ...
    for gambler in gamblers_list:
        environment.create_gambler(**{'nickname': gambler[0][0], 'email': gambler[0][1]})
    ...
    for tournament_data_index, tournament in enumerate(tournaments):
        ...
        info = environment.get_tournament_info(**{'league_index': coppa_cobram_index, 'tournament_index': tournament_index})
        draw_type = info['draw_type']
        ...
        environment.update_tournament(**{'league_index': coppa_cobram_index, 'tournament_index': tournament_index, 'is_open': False})

        print(name, year)

        tournament_index = tournament_id_to_index(coppa_cobram_index, (name, year))
        tournament_ranking = environment.get_tournament_ranking(**{'league_index': coppa_cobram_index, 'tournament_index': tournament_index})
        print(list((environment.get_gambler_info(index=index)['nickname'], score) for index, score in tournament_ranking['tournament_scores'].items()))
        print(list((environment.get_gambler_info(index=index)['nickname'], score) for index, score in tournament_ranking['tournament_ranking_scores'].items()))
        league_ranking = environment.get_league_ranking(**{'league_index': coppa_cobram_index})
        print(list((environment.get_gambler_info(index=index)['nickname'], score) for index, score in league_ranking['ranking_scores'].items()))
    ...
    macchia_index = gambler_nickname_to_index("Macchia")
    environment.update_gambler_in_league(league_index=coppa_cobram_index, gambler_index=macchia_index, is_active=False)
    ...
    tm = environment.get_tournament_matches(league_index=0, tournament_index=tournament_index)
    for ttm in tm:
        print(ttm)
        print(tm[ttm])
    tm = environment.get_tournament_bets(league_index=0, tournament_index=tournament_index, gambler_index=1)
    for ttm in tm:
        print(ttm)
        print(tm[ttm])

    tournament_index = tournament_id_to_index(coppa_cobram_index, ('Wimbledon Championships', 2021))
    print(environment.get_tournament_ranking(league_index=coppa_cobram_index, tournament_index=tournament_index))
